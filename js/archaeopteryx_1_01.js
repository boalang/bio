                ...
                if (n.properties && n.properties.length > 0) {
                    ...
                    for (i = 0; i < propertiesLength; ++i) {
                        ...
                        if (property.ref && property.value) {
                            ...
                            switch (property.ref){
                                case 'avg_exon':
                                    avg_exon=property.value;
                                    break;
                                case 'avg_gene':
                                    avg_gene=property.value;
                                    break;
                                case 'avg_mRNA':
                                    avg_mRNA=property.value;
                                    break;
                                case 'avg_CDS':
                                    avg_CDS=property.value;
                                    break;   
                                case 'CDS_no':
                                    CDS_no=property.value;
                                    break; 
                                case 'gene_no':
                                    gene_no=property.value;
                                    break; 
                                case 'exon_no':
                                    exon_no=property.value;
                                    break; 
                                case 'mRNA_no':
                                    mRNA_no=property.value;
                                    break;     
                                 }
                        }
...
//                $("<div id='" + NODE_DATA + "'>" + text + "</div>").dialog();
                ...
                var statData=[
                    {letter: "exon", frequency:avg_exon},
                    {letter: "gene", frequency:avg_gene},
                    {letter: "mRNA", frequency:avg_mRNA},
                    {letter: "CDS", frequency:avg_CDS},
                    
                ];
            
               var statData2=[
                    {letter: "exon_no", frequency:exon_no},
                    {letter: "gene_no", frequency:gene_no},
                    {letter: "mRNA_no", frequency:mRNA_no},
                    {letter: "CDS_no", frequency:CDS_no},
                ...
                var tip = d3.tip()
                      ...
                      .offset([-10, 0])
                      .html(function(d) {
                        return "<strong>Average:</strong> <span style='color:red'>" + d.frequency + "</span>";
                      })
                
                d3.select("#svgID").remove();
                var svg = d3.select("#controls2").append("svg")
                            .attr("id", "svgID")
...
//                d3.tsv("data.tsv", type, function(error, data) {
                  x.domain(statData.map(function(d) { return d.letter; }));
                  y.domain([0, d3.max(statData, function(d) { return d.frequency; })]);
...
//                      .text("Frequency");
            ...
            //second chart shows the number of each feature
              ...
              var margin = {top: 40, right: 20, bottom: 30, left: 40},
                ...
                d3.select("#svg2ID").remove();
                var svg = d3.select("#controls4").append("svg")
                            .attr("id", "svg2ID")
                            .attr("width", width + margin.left + margin.right)
                            .attr("height", height + margin.top + margin.bottom)
                            .append("g")
...
//                d3.tsv("data.tsv", type, function(error, data) {
                  x.domain(statData2.map(function(d) { return d.letter; }));
                  y.domain([0, d3.max(statData2, function(d) { return d.frequency; })]);
...
//
////                $("<div id='" + NODE_DATA + "'>" + text + "</div>").dialog();
////                window.alert("Node data showed!")
//                $("#controls3").html(text);
////                d3.select("#controls2").html("Node data has shown! by D3");
//                
//                var statData=[
//                    {letter: "exon", frequency:avg_exon},
//                    {letter: "gene", frequency:avg_gene},
