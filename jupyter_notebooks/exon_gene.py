    ...
    with open(BoaOutput+ "_modified", "w") as fouput:
        for key, value in gene_dic.items():
            if key in exon_dic:
                ...
                tmp_refseq = words[0]
                tmp_taxid = words [1]
                fouput.write(tmp_refseq+","+tmp_taxid+","+ value+","+ exon_dic[key]+"," + str(int(exon_dic[key])/int(value))[:5] + "\n")



def get_stats(assembly_file, intereseted_taxids_file):
    df_assemblystats=pd.read_csv(assembly_file, names=['refseq','taxid','gene','exon','exonpergene'])
